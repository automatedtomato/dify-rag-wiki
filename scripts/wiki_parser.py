...
SKIP_PREFIXES = ("Wikipedia:", "Help:", "Template:", "Portal:", "プロジェクト:", "ノート:", "ファイル:", "カテゴリ:")
...
def main():
    ...
    article_count = 0
    try:
        with bz2.open(XML_FILE_PATH, "rt", encoding="utf-8") as f_in, \
            ...
            for _, elem in tqdm(context, desc="Parsing XML to JSONL"):
                ...
                if tag == "page":
                    ...
                    # Robustness check
                    if (id_elem is not None and id_elem.text and
                        title_elem is not None and title_elem.text and
                        text_elem is not None and text_elem.text):
                        
                        title = title_elem.text
                        text = text_elem.text

                        # Skip redirect pages and meta-pages
                        if title.startswith(SKIP_PREFIXES) or text.strip().upper().startswith("#REDIRECT"):
                            elem.clear()
                            continue
        ...
        logger.info(f"Parsing complete. {article_count} articles written to {OUTPUT_JSONL_PATH}")
    except FileNotFoundError:
        logger.error(f"Input file not found: {XML_FILE_PATH}. Please run the download script first.")
        sys.exit(1)
